...
executable  = "C:\\dcraw.exe" # -y -i C:/test.exr C:/outs.tiff"
...
for path, dirs, files in walklevel(PATH):
	...
	if not path.endswith("tiff") and not path.endswith("jpg"):
		if not os.path.exists(path + "\\tiff"):
			os.makedirs(path + "\\tiff")
	#if path.endswith("tiff"):
	for filename in files:
		if filename.endswith('.dng'):
			fullpath = os.path.join(path, filename)
			newFilePath = path + "\\tiff\\" +  filename[:-4] + ".tiff"
			...
			command = executable + " -T " + "\"" + fullpath + "\""# "\"" + " " + scaling + " " + "\""+newFilePath + "\"" + " -y -loglevel panic"
			commandPool.append(command)
			#print command
			#break

print "=== going to processing parallel ==="
pool = Pool(26)
